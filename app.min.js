document.addEventListener('DOMContentLoaded', () => {
    ...
    d.addEventListener('change', () => {
        ...
        if (e.target.classList.contains('tab-button')) {
            o.querySelectorAll('.tab-button').forEach(e => e.classList.remove('active')), e.target.classList.add('active');
            const t = e.target.getAttribute('data-tab'),
                n = x.find(e => e.name.toLowerCase() === t).parser;
            l.innerHTML = l.querySelectorAll('.result-item').length ? Array.from(l.querySelectorAll('.result-item')).filter(e => e.getAttribute('data-category') === t).map(e => e.outerHTML).join('') : '<div class="result-item">कोई परिणाम नहीं।</div>'
        }
    }), document.addEventListener('click', e => {
        m.contains(e.target) || u.contains(e.target) || m.classList.remove('open')
    }), window.onscroll = () => {
        [h, f].forEach(e => e.style.display = document.documentElement.scrollTop > 50 ? 'block' : 'none'), document.documentElement.scrollTop + window.innerHeight >= document.documentElement.scrollHeight - 100 && k.currentPage * 10 < 50 && F()
    ...
    async function M(e) {
        ...
        if (/(weather|mausam)\s(in|mein)\s(.+)/i.test(t)) {
            ...
            return a.innerHTML = `<div class="widget-card"><h3>${n} का मौसम</h3><p>ऑफलाइन अनुमान: 25°C, साफ़</p></div>`, !0
        }
        if (/news\s(.+)/i.test(t)) {
            const n = t.match(/news\s(.+)/)[1];
            return a.innerHTML = `<div class="widget-card"><h3>${n} की खबरें</h3><p>मुफ्त डेटा के बिना ऑफलाइन सूचना: कोई लाइव अपडेट नहीं</p></div>`, !0
        }
        return !1
    }
    async function F() {
        ...
        try {
            ...
            if (i.web.length) {
                o.innerHTML = ['web', 'images', 'videos', 'news'].map(e => `<button class="tab-button ${'web'===e?'active':''}" data-tab="${e}">${'web'===e?'सभी':'images'===e?'चित्र':'videos'===e?'वीडियो':'news'===e?'खबरें':e}</button>`).join(''), l.innerHTML = i.web.slice(0, k.currentPage * k.resultsPerPage).map(e => `<div class="result-item" data-category="${e.category||'web'}">${e.thumbnail?`<img src="${e.thumbnail}" loading="lazy" alt="${e.title}">`:''}<div><a href="${e.link}" target="_blank">${e.title}</a><p>${e.snippet}</p><small>स्रोत: ${e.source}</small></div></div>`).join(''), await L(k.currentQuery, !0)
    ...
    function N(e) {
        const t = e.querySelectorAll('div.g');
        return Array.from(t).map(e => {
            const t = e.querySelector('h3'),
                n = e.querySelector('a'),
                s = e.querySelector('span.st');
            return t && n ? {
                title: t.textContent,
                link: n.href,
    ...
    function O(e) {
        const t = e.querySelectorAll('li.b_algo');
        return Array.from(t).map(e => {
    ...
    function A(e) {
        const t = e.querySelectorAll('div#dismissable');
        return Array.from(t).map(e => {
            const t = e.querySelector('a'),
                n = e.querySelector('h3'),
                s = e.querySelector('div#result-stats');
            return t && n ? {
                title: n.textContent,
                link: t.href,
                snippet: s?.textContent || '',
    ...
    }, P(), o.addEventListener('click', e => {
        if (e.target.classList.contains('tab-button')) {
            const t = e.target.getAttribute('data-tab');
            l.innerHTML = Array.from(l.querySelectorAll('.result-item')).filter(e => e.getAttribute('data-category') === t).map(e => e.outerHTML).join('') || '<div class="result-item">कोई परिणाम नहीं।</div>'
        }
    })
}), self.addEventListener('fetch', e => {
    e.respondWith(caches.match(e.request).then(t => t || fetch(e.request).then(t => {
        if (!t || 200 !== t.status || 'basic' !== t.type) return t;
        const n = t.clone();
        return caches.open('mistaFy-v1').then(e => e.put(e.request, n)), t
    })))
});⁸
